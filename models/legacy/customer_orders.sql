...
join (
      select 
        ...
        * 
      from {{ source('jaffle_shop_2', 'jaffle_shop_customers')  }}
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from {{ source('jaffle_shop_2', 'jaffle_shop_orders')  }}
    ) a
    ...
    left outer join {{ source('stripe_2', 'stripe_payments')  }} c
    on a.id = c.orderid
...
left outer join {{ source('stripe_2', 'stripe_payments')  }} payments
